...
- **GitHub URL Support** — Point it at a GitHub repo URL and it will clone and document it.
...
- **Quality Evaluation** — Built-in evaluator scores the generated documentation for coverage and quality.
- **Hierarchical Analysis** — Identifies the project's "north star", architecture style, heat zones, and entry points before writing.
- **Version History** — MongoDB integration stores every generated doc with diffs and version tracking.
...
├── core/
│   ├── loader.py        # Repository loading (local + GitHub clone)
│   ├── indexer.py        # Codebase indexing with LlamaIndex
│   ├── analyzer.py       # Hierarchical codebase analysis
...
│   ├── chat.py           # Interactive Q&A engine
│   └── db.py             # MongoDB version store
├── config/
...
```
Load Repo → Index Codebase → Hierarchical Analysis → Plan Sections → Write Docs → Evaluate Quality → Save
```
...
- **Git** (required if documenting GitHub URLs)
...
- Configure the LLM model and target scope
- View the generation pipeline in real time
- Browse and diff past documentation versions (requires MongoDB)
- Chat with the codebase interactively
